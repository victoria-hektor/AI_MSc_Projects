...
class PFLocaliser(PFLocaliserBase):
    ...
    def log_localisation_errors(self):
        """
        Calculates the localisation error between estimated poses and ground truth poses
        and logs the mean error to the console.
        """
        errors = []
        for est, truth in zip(self.estimated_poses, self.ground_truth_poses):
            error = self.calculate_distance(est.position, truth.position)
            errors.append(error)
        # Calculate the mean error for all estimated poses if there are any errors
        mean_error = sum(errors) / len(errors) if errors else 0
        rospy.loginfo(f"Mean localization error: {mean_error:.3f} meters")
    ...
    def __init__(self):
        ...
        self.ground_truth_poses = [] # for visualisation and error logging
        self.estimated_poses = [] # for error logging

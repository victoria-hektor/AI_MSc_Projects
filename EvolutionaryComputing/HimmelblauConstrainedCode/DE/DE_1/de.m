%% DE /best/1/bin with Dynamic Parameter Testing incorporating a halt 10 iterations after achieving the global minimum within specified tolerance
clc; clear;
...
prevBestCost = inf;                % Initialise with a high value assuming minimization
...
for run = 1:length(parameterSets)
    ...
    for it = 1:params.MaxIt
		for i = 1:params.nPop
			...
			b = A(2);
			c = A(3);
			y = pop(a).Position + params.beta_min*(pop(b).Position - pop(c).Position);
			y = max(y, VarMin);
			y = min(y, VarMax);

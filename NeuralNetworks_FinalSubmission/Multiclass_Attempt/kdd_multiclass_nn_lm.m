...
try
    ...
    output = categorical(data(:,41)); % I use 'categorical' for multi-class classification of attack column
catch
...
for window = 1:numWindows % This is based on the number of windows set earlier
    try
    ...
    for r = 1:10
        try
            ...
            disp(r*2)
            % Set ANN parameters
            net.trainFcn = 'trainlm'; % Use 'trainlm' for Levenberg-Marquardt backpropagation
            net.trainParam.epochs = r * 10; % Set the maximum number of training iterations
            net.trainParam.lr = 0.06; % Learning rate
            net.performFcn = 'crossentropy'; % Set the performance function for multi-class classification
            ...
            % Calculate the performance of the network using cross-entropy for this window
            perf = perform(net, outputTest, annoutputs);
            perfArray(window, r) = perf;
            
            % If loop to track the lowest error
            if perf < lowestError % If better than the previous, updates below
                bestConfig.layers = r;
...
try
    ...
    error('Error displaying the best configuration and lowest error.'); % Error message
end
...
try
    ...
    disp(averagePerformance); % Calls on the calculated performance for display
catch
    error('Error calculating average performance.'); % Error message
end

% Create a line plot to depict the performance for each step
x = 2:2:20; % Assuming there are 10 iterations with r ranging from 1 to 10 - change this if parameters change
...
try
    ...
    ylabel('Average Performance (Cross-Entropy)');
    grid on;
catch
